    ...
    private void readMessages() throws IOException {
        while (true) {
            ...
            if(messageInChat.equals(ServerCommandConstants.SHUTDOWN)) {
                return;
            } else if (messageInChat.contains(ServerCommandConstants.SPECIFICUSER)) {
                String nik = messageInChat.trim().split(" ")[1];
                messageInChat = messageInChat.replace(ServerCommandConstants.SPECIFICUSER,"")
                                .replace(nik,"").trim();
                server.broadcastMessage(nickName + ": " + messageInChat, nik);
            }
